...
GpuBufferHandle OpenGLBackend::create_buffer(std::size_t sizeBytes) {
    ...
    glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizei>(sizeBytes), nullptr, GL_DYNAMIC_DRAW);
...
void OpenGLBackend::upload(GpuBufferHandle handle,
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, static_cast<GLsizeiptr>(count * sizeof(float)), data);
}
/*
